...
st.markdown(
    ...
    div[role="radiogroup"] label {
        ...
        padding: 10px 20px;
        border-radius: 8px;
        margin: 0;       /* Убираем лишние маржины, используем gap */
        border: 1px solid #444;
        transition: 0.3s;
        flex: 1 0 auto;   /* Растягиваемся, но не сжимаемся меньше min-width */
        min-width: 120px; /* Минимальная ширина, чтобы текст не сплющивался */
        text-align: center;
        justify-content: center;
    }
...
def tab_dashboard() -> None:
    ...
    # Контейнер снижает риск наложения при динамических rerun.
    with st.container():
        if G_view is None:
            return
...
def tab_energy() -> None:
    ...
    # Контейнер предотвращает смешивание блоков при активных rerun.
    with st.container():
...
def tab_structure() -> None:
    ...
    # Контейнер для 3D и матрицы смежности (предотвращает визуальные "слипания").
    with st.container():
        if G_view is None:
            return
        ...
        with col_vis_main:
            ...
            with st.spinner("Рендеринг 3D сцены..."):
...
def tab_null_models() -> None:
    ...
    # Контейнер удерживает блоки null-моделей в одном DOM-узле.
    with st.container():
        if G_view is None:
            return
...
def tab_attack_lab() -> None:
    ...
    # Контейнер упрощает перерисовку сложного UI.
    with st.container():
        if G_view is None:
            return
...
def tab_compare() -> None:
    ...
    # Контейнер снижает визуальные артефакты при переключении режимов.
    with st.container():
        if G_view is None:
            return
